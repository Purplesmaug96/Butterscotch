...
static void d3d9DrawSprite(Renderer* renderer, int32_t tpagIndex, float x, float y,
	...
	if (!dr->textures[texPageId]) {
		D3D9_DIAG_LIMITED(64, "D3D9: drawSprite null texture page=%d tpag=%d\n", texPageId, tpagIndex);
		return;
	}
...
static void d3d9DrawSpritePart(Renderer* renderer, int32_t tpagIndex,
	...
	if (!dr->textures[texPageId]) {
		D3D9_DIAG_LIMITED(64, "D3D9: drawSpritePart null texture page=%d tpag=%d\n", texPageId, tpagIndex);
		return;
	}
...
static void d3d9DrawSpritePos(Renderer* renderer, int32_t tpagIndex, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, float alpha) {
	...
	if (0 > tpagIndex || (uint32_t)tpagIndex >= dw->tpag.count) {
		D3D9_DIAG_LIMITED(64, "D3D9: drawSpritePos invalid tpag=%d count=%u\n", tpagIndex, dw->tpag.count);
		return;
	}
	TexturePageItem* tpag = &dw->tpag.items[tpagIndex];
	int32_t texPageId = tpag->texturePageId;
...
static bool d3d9SetRenderTarget(Renderer* renderer, int32_t surfaceID, bool implicitApplicationSurface) {
	...
	if (surfaceID == APPLICATION_SURFACE_ID) {
		if (implicitApplicationSurface && dr->savedViewStateValid) {
			...
			if (FAILED(hr)) {
				D3D9_DIAG_LIMITED(64, "D3D9: SetRenderTarget(app_surface implicit) failed hr=0x%08X\n", (unsigned)hr);
				return false;
			}
			D3DVIEWPORT9 vp;
			vp.X = 0;
		...
		HRESULT hr = dev->SetRenderTarget(0, (IDirect3DSurface9*)dr->appSurfaceLevel);
		if (FAILED(hr)) {
			D3D9_DIAG_LIMITED(64, "D3D9: SetRenderTarget(app_surface) failed hr=0x%08X\n", (unsigned)hr);
			return false;
		}
		D3DVIEWPORT9 vp;
		vp.X = 0;
	...
	if (surfaceID < 0 || (uint32_t)surfaceID >= dr->surfaceCount || !dr->surfaces[surfaceID]) {
		D3D9_DIAG_LIMITED(32, "D3D9: surface_set_target invalid id=%d\n", surfaceID);
		return false;
	}
	...
	if (FAILED(hr)) {
		D3D9_DIAG_LIMITED(64, "D3D9: SetRenderTarget(surface %d) failed hr=0x%08X\n", surfaceID, (unsigned)hr);
		return false;
	}
...
static void d3d9DrawSurface(Renderer* renderer, int32_t surfaceID, int32_t srcLeft, int32_t srcTop,
	...
	if (surfaceID == APPLICATION_SURFACE_ID) {
		if (!dr->appSurfaceTexture) {
			D3D9_DIAG_LIMITED(64, "D3D9: draw_surface no app surface id=%d\n", surfaceID);
			return;
		}
...
static bool d3d9SurfaceGetPixels(Renderer* renderer, int32_t surfaceID, uint8_t* outRGBA) {
	...
	D3D9_DIAG_LIMITED(16, "D3D9: surface_get_pixels %d (%dx%d) ok\n", surfaceID, w, h);
	return true;
}
...
static void ensureShaderCompiled(D3D9Renderer* dr, int32_t shaderIndex) {
	...
	if (debugMode) fprintf(stderr, "D3D9: ===== VERTEX SOURCE for %s =====\n%s\n", shdr->name, vertexShaderSource);
	if (debugMode) fprintf(stderr, "D3D9: ===== FRAGMENT SOURCE for %s =====\n%s\n", shdr->name, fragmentShaderSource);
