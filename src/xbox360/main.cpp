...
static void _diagLog(FILE* file, const char* fmt, va_list args) {
	...
	vsnprintf(line, sizeof(line) - 2, fmt, args);
	...
	if (copyLen >= FATAL_LOG_LINE_LEN) {
		copyLen = FATAL_LOG_LINE_LEN - 1;
	}
...
static void diagOpenLog(void) {
	if (gDiagLog != INVALID_HANDLE_VALUE || gDiagFile || gDiagTriedFallback) {
		return;
	}
...
static void loadingDestroy(LoadingScreen* ls) {
	...
	if (ls->vertexShader) {
		ls->vertexShader->Release();
	}
	if (ls->pixelShader) {
		ls->pixelShader->Release();
	}
	if (ls->vertexDecl) {
		ls->vertexDecl->Release();
	}
	memset(ls, 0, sizeof(*ls));
}
...
static void loadingDrawText(LoadingScreen* ls, const char* text, float x, float y, float scale,
	...
	for (const char* p = text; *p; p++) {
		...
		if (ch < DEBUGFONT_FIRST_CP || ch > DEBUGFONT_LAST_CP) {
			ch = '?';
		}
...
static void diagOverlayDraw(Runner* runner, Renderer* renderer, int32_t frameW, int32_t frameH) {
	if (!gDiagOverlayVisible || !runner || !gDiagOverlayScreen.available) {
		return;
	}
...
static void setGamepadButton(GamepadSlot* slot, int index, bool down) {
	if (index < 0 || index >= GP_BUTTON_COUNT) {
		return;
	}
...
static void pollGamepadApi(Runner* runner, const XINPUT_STATE* state, WORD buttons, bool connected) {
	...
	for (int i = 0; i < GP_BUTTON_COUNT; i++) {
		...
		if (slot->buttonDown[i] && !wasDown) {
			slot->buttonPressed[i] = true;
		}
		if (!slot->buttonDown[i] && wasDown) {
			slot->buttonReleased[i] = true;
		}
	}
...
static void _main() {
	try {
		...
		parseOpts.parseGen8 = true;
		parseOpts.parseOptn = true;
		parseOpts.parseLang = true;
		parseOpts.parseExtn = true;
		parseOpts.parseSond = true;
		parseOpts.parseAgrp = true;
		parseOpts.parseSprt = true;
		parseOpts.parseBgnd = true;
		parseOpts.parsePath = true;
		parseOpts.parseScpt = true;
		parseOpts.parseGlob = true;
		parseOpts.parseShdr = true;
		parseOpts.parseFont = true;
		parseOpts.parseTmln = true;
		parseOpts.parseObjt = true;
		parseOpts.parseRoom = true;
		parseOpts.parseTpag = true;
		parseOpts.parseCode = true;
		parseOpts.parseVari = true;
		parseOpts.parseFunc = true;
		parseOpts.parseStrg = true;
		parseOpts.parseTxtr = true;
		parseOpts.parseAudo = true;
		...
		*lastSlash2 = '\0';
		...
		diagLog("Butterscotch: (08b) loading SHADERS.BIN\n");
		{
			...
			if (lastSlash) {
				size_t dirLen = (size_t)(lastSlash - dataWinPath + 1);
				if (dirLen < sizeof(shadersPath) - 16) {
					memcpy(shadersPath, dataWinPath, dirLen);
					strcpy(shadersPath + dirLen, "SHADERS.BIN");
				} else {
					strcpy(shadersPath, "butterscotch:\\SHADERS.BIN");
				}
			} else {
				strcpy(shadersPath, "butterscotch:\\SHADERS.BIN");
			}

			FILE* shadersFile = fopen(shadersPath, "rb");
			if (shadersFile) {
				fseek(shadersFile, 0, SEEK_END);
				long shadersSize = ftell(shadersFile);
				fseek(shadersFile, 0, SEEK_SET);
				if (shadersSize > 0 && shadersSize <= 64 * 1024 * 1024) {
					uint8_t* shadersData = (uint8_t*)safeMalloc((size_t)shadersSize);
					if (shadersData) {
		...
		if (configRoot) {
			...
			if (gamepadApiVal) {
				if (JsonReader_isBool(gamepadApiVal)) {
					...
					if (enabledVal) {
						gamepadApiEnabled = JsonReader_getBool(enabledVal);
					}
				}
			}
		}
...
#define STICK_DEADZONE 16384
			...
			if (speedCapRemoved && targetFrameTime > accumulator) {
				accumulator = targetFrameTime;
			}
			...
			if (gameFramesRan > 0) {
				...
				if (gDiagOverlayWindowStart <= 0.0) {
		...
		if (nextWorkingDirectory != nullptr && nextLaunchParameters != nullptr) {
			...
			if (lastSlash) {
				size_t dirLen = (size_t)(lastSlash - dataWinPath + 1);
				if (dirLen >= sizeof(currentDataWinDir)) {
					dirLen = sizeof(currentDataWinDir) - 1;
				}
