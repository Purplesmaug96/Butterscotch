...
#if false // #ifdef ENABLE_LEGACY_GL
...
int main(int argc, char* argv[]) {
    ...
    Renderer* renderer;
#if false /* #ifdef ENABLE_LEGACY_GL */ && defined(ENABLE_SW_RENDERER)
    if(useSWRend)
        renderer = SWRenderer_create();
    else
        renderer = GLLegacyRenderer_create();
#elif false /* #ifdef ENABLE_LEGACY_GL */
    renderer = GLLegacyRenderer_create();
#else
    renderer = SWRenderer_create();
#endif
...
#if defined(ENABLE_SW_RENDERER) && false /* #ifdef ENABLE_LEGACY_GL */
        if(useSWRend)
