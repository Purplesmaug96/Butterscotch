...
static uint8_t* rleDecompress(const uint8_t* rleData, size_t rleSize, size_t expectedOutputSize) {
	if (!rleData || rleSize == 0 || expectedOutputSize == 0) {
		return NULL;
	}
...
bool Xbox360Textures_getTpagAtlasInfo(int32_t tpagIndex,
	...
	if (te->atlasId == 0xFFFF) {
		return false;
	}
	if (te->atlasId >= gAtlasCount) {
		return false;
	}

	if (outAtlasId) {
		*outAtlasId = (int)te->atlasId;
	}
	if (outAtlasX) {
		*outAtlasX = (int)te->atlasX;
	}
	if (outAtlasY) {
		*outAtlasY = (int)te->atlasY;
	}
	if (outWidth) {
		*outWidth = (int)te->width;
	}
	if (outHeight) {
		*outHeight = (int)te->height;
	}
	if (outClutIndex) {
		*outClutIndex = (int)te->clutIndex;
	}
	if (outBpp) {
		*outBpp = (int)te->bpp;
	}
...
static uint8_t* readAtlasPixelData(uint32_t atlasId, size_t* outSize) {
	if (!gInitialized || atlasId >= gAtlasCount || !gTexturesFile || !outSize) {
		return NULL;
	}
	*outSize = 0;
	...
	if (fseek(gTexturesFile, (long)ai->fileOffset, SEEK_SET) != 0) {
		return NULL;
	}
	...
	if (ai->compressionType == 1) {
		...
		if (uncompressedSize == (size_t)ai->pixelDataSize) {
			...
			if (result) {
				memcpy(result, compressed, uncompressedSize);
			}
		} else {
			...
			if (result) {
				...
				if (copySize < uncompressedSize) {
					memset(result + copySize, 0, uncompressedSize - copySize);
				}
			}
		}
	}
...
static const uint8_t* getClut8Colors(int clutIndex) {
	if (!gClut8Data || clutIndex < 0 || (uint32_t)clutIndex >= gClut8Count) {
		return NULL;
	}
...
bool Xbox360Textures_loadAtlasById(int atlasId, int* outW, int* outH, uint8_t** outPixels) {
	...
	*outPixels = NULL;
	...
	if (!gInitialized || atlasId < 0 || (uint32_t)atlasId >= gAtlasCount) {
		return false;
	}
