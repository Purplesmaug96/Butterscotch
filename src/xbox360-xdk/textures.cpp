...
bool Xbox360Textures_init(const char* texturesBinPath) {
    ...
    if (!gTpagClutMap) { free(mapBuf); goto fail; }
    ...
    if (!gPageInfo) { free(hdrBuf); goto fail; }
    ...
    if (fseek(gFp, (long)clutBytes, SEEK_CUR) != 0) goto fail;
...
fail:
    if (gFp) fclose(gFp);
    gFp = NULL;
    free(gTpagClutMap); gTpagClutMap = NULL;
    free(gPageInfo); gPageInfo = NULL;
    gClutCount = 0;
    gPageCount = 0;
    gTpagCount = 0;
    gPixelBlockBase = 0;
...
bool Xbox360Textures_loadPage(uint32_t pageId, int* outW, int* outH, uint8_t** outPixels) {
    if (!gInitialized || pageId >= gPageCount || !outW || !outH || !outPixels) return false;
